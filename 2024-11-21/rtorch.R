...
time = function(config) {
  ...
  nn_sequential2 = nn_module("nn_sequential2",
    ...
    forward = function(input) {
      self$tensors = list(input)
      for (module in private$modules_) {
        self$tensors[[length(self$tensors) + 1]] <- module(self$tensors[[length(self$tensors)]])
      }
      self$tensors[[length(self$tensors)]]
    }
  })
  # set cuda seed
  ...
  if (config$type == "ignite") {
    if (config$optimizer == "adamw") {
      opt = ignite::optim_ignite_adamw(net$parameters)
    } else {
      stop("not implemented yet")
    }
  } else {
    if (config$optimizer == "adamw") {
      opt = torch::optim_adamw(net$parameters)
    } else {
      stop("not implemented yet")
    }
  }
...
write.csv(config_tbl, here("2024-11-21", "R"))
